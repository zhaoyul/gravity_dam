...
- *温度分布云图*:
  在ParaView中, 我们可以直接选择 ~temp~ 变量来可视化大坝内部的稳态温度分布. 能观察到从上游冷水面到下游暖空气面的平滑温度过渡.

  温度分布云图:
  [[file:imgs/temperature.png]]

- *热应力分析*:
  温度梯度引起的热应力将叠加在原有的自重和水压应力之上. 通过对比耦合分析与纯力学分析的应力结果(例如 ~stress_yy~), 可以清晰地评估温度效应带来的影响. 通常, 上游冷却区域会产生拉应力, 而内部较暖区域则会产生压应力.

  由温度效应引起的应力增量或总应力云图:
  [[file:imgs/stress_yy_by_temperature.png]]



我们证明了自研平台不仅能处理单一物理场问题, 更具备了解决工程实际中常见的多物理场耦合问题的强大能力, 为进行更复杂, 更真实的工程模拟奠定了基础.
